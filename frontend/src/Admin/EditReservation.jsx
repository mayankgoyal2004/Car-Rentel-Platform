...
const AdminEditReservation = () => {
  ...
  const fetchReservationData = async () => {
    try {
      const res = await apiService.getsinglereservaton(id);
      if (res.data.success) {
    ...
    try {
      ...
      if (res.data.success) {
        const formattedCars = res.data.data.map((car) => ({
          ...
          year: car.year,
          price: car.pricing?.prices?.daily || 0,
          status: car.status,
          inRental: car.inRent,
  ...
  const handleSubmit = async () => {
    try {
      const formattedData = {
        ...
        baseKilometers: reservationData.baseKilometers,
        kmExtraPrice: reservationData.kmExtraPrice,
      };
